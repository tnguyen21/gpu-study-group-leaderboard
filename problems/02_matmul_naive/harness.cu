...
// Validation tests on critical edge cases
{
    int test_cases[][3] = {
        ...
        {33, 33, 33},        // Non-multiple of block size (16)
        {1, 256, 256},       // Single row output
    };
...
// Main test case for timing (square 1024x1024)
...
// Compute reference on CPU (simplified - just check a few elements)
for (int row = 0; row < M; row++) {
    for (int col = 0; col < N; col++) {
        float sum = 0.0f;
        for (int k = 0; k < K; k++) {
